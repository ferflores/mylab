function Spider(){
	...
	this.run = function(canvas, canvasWidth, canvasHeight){
		...
		require(["/js/classes/ParticlesOpt/Particle.js"], function(Particles) {
			_this.configure();
			_this.main();
		});
	}
	...
	this.createSpider = function(x,y){
		...
		arania.create(x,y,_this.canvas.height/(Math.random()*40+20), randomWalkProb);
	...
	function Arania(){
		...
		this.create = function(x, y, size, walkProb){
		...
		this.draw = function(context){
			...
			if(this.headParticle.y + this.radious > _this.floorY){
				this.headParticle.y = _this.floorY - this.radious;
		...
		this.walk = function(){
			if(this.headParticle.y > _this.floorY - 30){
		...
		this.drawLegs = function(c){
			...
			for (var i = 0; i < this.legs.length; i++) {
				...
				if(p1.y > _this.floorY){
					p1.y = _this.floorY;
					p1.setSpeed(p1.getSpeed()*p1.bounce*0.2);
				}

				if(p2.y > _this.floorY){
					p2.y = _this.floorY;
